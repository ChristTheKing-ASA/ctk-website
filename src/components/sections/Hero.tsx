...
import { getChurchInfo, getHomePage } from "@/lib/content";
import { Icon } from "@/lib/icons";
import { Play } from "lucide-react";

export async function Hero() {
  const [churchInfo, home] = await Promise.all([getChurchInfo(), getHomePage()]);
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 sm:py-16">
            ...
            {pillars.map((pillar) => (
              ...
              >
                <div className="w-12 h-12 bg-gold-500/30 rounded-full flex items-center justify-center mx-auto mb-4">
                  <Icon name={pillar.icon} className="w-6 h-6 text-gold-300" />
                </div>
        ...
        <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-3 sm:gap-4">
          {home?.heroPrimaryCtaLabel && (
            <Button
              href={home.heroPrimaryCtaHref || "/visit"}
              size="lg"
              variant="secondary"
            >
              {home.heroPrimaryCtaLabel}
          ...
          {home?.heroSecondaryCtaLabel && (
            <Button
              href={home.heroSecondaryCtaHref || "/worship/sermons"}
              size="lg"
              variant="ghost"
